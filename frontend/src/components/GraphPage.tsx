...
function GraphCanvas({ graph }: { graph: any }) {
  const lifecycleTypes = ['requirement', 'feature', 'variant', 'test', 'product']
  const nodes = [...(graph?.nodes || [])].sort((a, b) => Number(lifecycleTypes.includes(b.type)) - Number(lifecycleTypes.includes(a.type))).slice(0, 100)
  ...
  const edges = [...(graph?.edges || [])]
    ...
    .sort((a, b) => Number(b.properties?.origin === 'lifecycle_assertion') - Number(a.properties?.origin === 'lifecycle_assertion'))
    .slice(0, 220)
  ...
  const color = (type: string) => lifecycleTypes.includes(type) ? ['#ffe4e6', '#be123c'] : ['function', 'method'].includes(type) ? ['#dcfce7', '#16a34a'] : type === 'class' ? ['#ede9fe', '#7c3aed'] : type === 'package' ? ['#fef3c7', '#d97706'] : ['#dbeafe', '#2563eb']
  return <div className="h-[72vh] min-h-[620px] w-full overflow-auto rounded-xl border border-slate-200 bg-white">
    {(graph.nodes.length > nodes.length || graph.edges.length > edges.length) && <p className="p-2 text-sm text-amber-800">Preview shows {nodes.length} of {graph.nodes.length} nodes and {edges.length} of {graph.edges.length} relationships. Download the full graph for all records.</p>}
...
export default function GraphPage({ onBack }: GraphPageProps) {
  ...
  const generate = async () => {
    ...
    try {
      ...
      const source = local ? { files: await readFiles(files) } : { project_url: gitlabUrl.trim() }
      if (manifestFile && manifestFile.size > 1_000_000) throw new Error('Lifecycle manifest must be under 1 MB.')
      const payload = manifestFile ? { source, manifest: JSON.parse(await manifestFile.text()) } : source
      const response = await fetch(manifestFile ? API_CONFIG.GRAPH.TRACE : local ? API_CONFIG.GRAPH.LOCAL : API_CONFIG.GRAPH.GITLAB, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
      const result = await response.json()
      if (!response.ok) throw new Error(typeof result.error === 'string' ? result.error : typeof result.detail === 'string' ? result.detail : JSON.stringify(result.detail || 'Graph generation failed'))
      setGraph(result)
  ...
  return <main className="min-h-screen bg-slate-50 p-6 lg:p-8"><div className="mx-auto max-w-[1600px]">
    ...
    <section className="mt-4 rounded-xl border border-slate-200 bg-white p-5">
      <label className="block text-sm font-medium text-slate-700">Lifecycle links (optional JSON manifest)
        <input type="file" accept=".json,application/json" onChange={event => setManifestFile(event.target.files?.[0] || null)} className="mt-2 block" />
      </label>
      <p className="mt-2 text-sm text-slate-500">Add requirement, feature, variant, test and product links to the source graph. Imported lifecycle nodes appear in rose.</p>
      {files.length > 0 && <button onClick={() => setFiles([])} className="mt-2 mr-4 text-sm text-indigo-700">Use GitLab instead</button>}
      {manifestFile && <button onClick={() => setManifestFile(null)} className="mt-2 text-sm text-indigo-700">Remove lifecycle links</button>}
      {graph && <button onClick={() => { const url = URL.createObjectURL(new Blob([JSON.stringify(graph, null, 2)], { type: 'application/json' })); const link = document.createElement('a'); link.href = url; link.download = 'codetrace-graph.json'; link.click(); setTimeout(() => URL.revokeObjectURL(url), 1000) }} className="ml-4 text-sm text-indigo-700">Download full graph</button>}
    </section>
    {graph?.metadata?.diagnostics?.length > 0 && <p className="mt-3 text-sm text-amber-800">{graph.metadata.diagnostics.length} lifecycle resource(s) have links but no imported details. See graph export for identifiers.</p>}
    {error && <p className="mt-4 rounded-lg border border-red-200 bg-red-50 p-3 text-red-700">{error}</p>}
    {graph && <section className="mt-6"><div className="mb-3 flex flex-wrap gap-5 text-sm text-slate-600"><span><b>{graph.nodes?.length || 0}</b> nodes</span><span><b>{graph.edges?.length || 0}</b> edges</span><span>{graph.metadata?.source === 'combined_trace' ? 'Code + lifecycle knowledge graph' : graph.metadata?.source === 'gitlab' ? 'GitLab project' : 'Local folder'}</span></div><GraphCanvas graph={graph} /></section>}
