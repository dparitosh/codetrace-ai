...
class CodeTraceGraphAnalyzer:
    ...
    def _compute_complexity_metrics(self, nx_graph) -> Dict[str, Any]:
        ...
        try:
            ...
            # MultiDiGraph preserves semantic edges, while these aggregate
            # algorithms expect a simple graph in several NetworkX versions.
            metric_graph = nx.DiGraph(nx_graph)

            metrics = {
                "density": nx.density(metric_graph),
                "is_connected": bool(metric_graph) and nx.is_weakly_connected(metric_graph),
                "number_of_components": nx.number_weakly_connected_components(metric_graph) if metric_graph else 0,
                "average_clustering": nx.average_clustering(nx.Graph(metric_graph)) if metric_graph else 0.0,
            }
            
            # Centrality measures for top nodes
            centrality = nx.degree_centrality(metric_graph)
