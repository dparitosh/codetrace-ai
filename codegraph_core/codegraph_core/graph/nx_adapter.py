...
def to_networkx(inmem_graph, *, multigraph: bool = True) -> Optional[object]:
    """Convert the core graph without losing parallel relationship types."""
    try:
    ...
    for src, out_edges in inmem_graph.edges.items():
        for (dst, etype, attrs) in out_edges:
            edge_attrs = {"type": etype, **attrs}
            if multigraph:
                # Let NetworkX allocate a unique key; the relationship type is
                # stored as data because the same type can occur more than once.
                G.add_edge(src, dst, **edge_attrs)
            elif not G.has_edge(src, dst):
                G.add_edge(src, dst, **edge_attrs)
    return G
...
def compute_basic_metrics(G, focus_nodes: List[str]) -> Dict[str, Dict[str, float]]:
    ...
    if len(G) == 0:
        return {n: {"in_degree": 0.0, "out_degree": 0.0, "degree_centrality": 0.0}
